...
def metric_selection():
    met_ind_cluss = st.selectbox(
        "Induction mearure", measure_selection, index=6)
...
def regression_param():
    dictionary = {"mean_based_regression": st.toggle(
...
def define_params_class():
    ...
    if on_expert is False:
        ...
        if expert_params["extend_using_preferred"]:
            st.session_state.pref_list, st.session_state.forb_list = define_prefered_extend(
        ...
        if expert_params["induce_using_preferred"]:
            st.session_state.ind_exp_list = define_prefered_induction(
                st.session_state.ind_exp_list)
        else:
            st.session_state.ind_exp_list = []
    ...
    with tab3:
        ...
        if st.session_state.gn and st.session_state.button_rule:
            ...
            if eval_type == "Only training" or eval_type == "Training and testing - Hold out":
                ...
                # placceholder is used to updated the table with rules during the rule induction process
